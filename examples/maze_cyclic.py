...
class TrainState(BaseTrainState):
    ...
    level_history: core.FrozenDict[str, chex.ArrayTree] = struct.field(pytree_node=True)
...
def train_state_to_log_dict(train_state: TrainState, level_sampler: CyclicLevelSampler) -> dict:
    """To prevent the entire (large) train_state to be copied to the CPU when doing logging, this function returns alvc of the important information in a dictionary format.
...
def main(config=None, project="JAXUED_TEST"):
    ...
    def log_eval(stats, train_state_info):
        ...
        print(f"Logged update: {stats['update_count']} in {end - start :.1f}s")
    ...
    # And the level sampler + history
    level_sampler = CyclicLevelSampler(
        sample_random_level,
        n_levels_per_step=config['num_train_envs'],
        n_levels_per_phase=config['n_levels_per_phase'],
        n_steps_per_phase=config['n_steps_per_phase'],
        n_phases=config['n_phases']
    )
    level_history = LevelHistory(config['num_updates'], config['num_train_envs'])
    
    @jax.jit
    def create_train_state(rng) -> TrainState:
        # Creates the train state
        rng, level_creation_key = jax.random.split(rng)
        ...
        sampler = level_sampler.initialize(level_creation_key)
        history = level_history.initialize(pholder_level)
    ...
    @jax.jit
    def train_step_on_levels(carry: Tuple[chex.PRNGKey, TrainState, chex.ArrayTree], _):
        rng, train_state, levels = carry
        ...
        history, idxs = level_history.insert_batch(train_state.level_history, levels)
        ...
        return (rng, train_state, levels), metrics
        
    # region train step
    @jax.jit
    def train_step(carry: Tuple[chex.PRNGKey, TrainState], _):
        """
            Sample levels and train on them
        """
        rng, train_state = carry        
        sampler, (level_inds, levels) = level_sampler.get_levels(train_state.sampler)
        train_state = train_state.replace(sampler=sampler)
        (rng, train_state, levels), metrics = train_step_on_levels((rng, train_state, levels), None)
    ...
    @jax.jit
    def eval_on_level(rng: chex.PRNGKey, train_state: TrainState, level: chex.ArrayTree):
        init_obs, init_env_state = jax.vmap(eval_env.reset_to_level, (0, None, None))(jax.random.split(rng, config['eval_num_attempts']), level, env_params)
        states, rewards, episode_lengths = evaluate_rnn(
            rng,
            eval_env,
            env_params,
            train_state,
            ActorCritic.initialize_carry((config['eval_num_attempts'],)),
            init_obs,
            init_env_state,
            env_params.max_steps_in_episode,
        )
    ...
    if config['mode'] == 'eval':
        start = time.time()
        c = eval_checkpoint(config) # evaluate and exit early
        end = time.time()
    ...
    rng_init, rng_train, rng_eval = jax.random.split(rng, 3)
    
    train_state = create_train_state(rng_init)
    runner_state = (rng_train, train_state)


    # Collect baselines for each level by training a dedicated agent on just that level
    @jax.jit
    def _train_and_eval_step_on_level(_, level):
        train_levels = jax.tree.map(lambda x : jnp.expand_dims(x, 0).repeat(config['num_train_envs'], axis=0), level)
        (_, final_train_state, _), metrics = jax.lax.scan(train_step_on_levels, (rng_train, train_state, train_levels), None, config['n_baseline_train_steps'])
        states, cum_rewards, ep_lengths = eval_on_level(rng_eval, final_train_state, level)
        return None, cum_rewards.mean()
    
    start = time.time()
    all_levels = level_sampler.get_all_levels(train_state.sampler)
    _, baseline_rewards = jax.lax.scan(_train_and_eval_step_on_level, None, all_levels)
    # baseline_rewards = jnp.zeros((level_sampler.n_levels,)) # dummy baselines if you cba to wait
    end = time.time()
    wandb.log({
        f'level-{i}/baseline': baseline_rewards[i]
        for i in range(baseline_rewards.shape[0])
    })
    print(f"Produced baselines in {end - start:.2f}s = {(end-start)/level_sampler.n_levels :.2f}s/level")
    ...
    for eval_step in range(config["num_updates"] // config["eval_freq"]):
        ...
        print('Processing level history...')
        level_history.print_stats(runner_state[1].level_history)

        print('Performing plasticity eval')
        eval_levels = level_sampler.get_all_levels(train_state.sampler)
        pl_states, pl_cum_rewards, pl_episode_lengths = jax.vmap(eval_on_level, (0, None, 0))(jax.random.split(rng_eval, level_sampler.n_levels), runner_state[1], eval_levels)
        pl_level_mean_rewards, pl_level_ep_lengths = pl_cum_rewards.mean(axis=-1), pl_episode_lengths.mean(axis=-1)
        visit_counts = level_history.get_visit_counts(runner_state[1].level_history)
        
        print(f'[step: {eval_step}] {pl_level_mean_rewards}')
        
        pl_eval_metrics = {}
        pl_eval_metrics['num_updates'] = (eval_step+1) * config['eval_freq']
        for i in range(pl_level_mean_rewards.shape[0]):
            pl_eval_metrics[f'level-{i}/score'] = pl_level_mean_rewards[i]
            pl_eval_metrics[f'level-{i}/normalised_score'] = pl_level_mean_rewards[i] - baseline_rewards[i]
        for i in range(visit_counts.shape[0]):
            pl_eval_metrics[f'level-{i}/visit_count'] = visit_counts[i]
        
        wandb.log(pl_eval_metrics)
...
if __name__=="__main__":
    ...
    # === Cyclic Sampler ===
    parser.add_argument("--n_baseline_train_steps", type=int, default=200)
    parser.add_argument("--n_levels_per_phase", type=int, default=1)
    parser.add_argument("--n_steps_per_phase", type=int, default=50)
